...
def metadata_applier(
    tags: Tags, fixed_location: Path, exclude_tags: list[str], fallback_mv=True
):
    ...
    for k, v in tags.items():
        ...
        if isinstance(v, list):
            if not fallback_mv or (k not in fallback_mv_keys):
                setattr(
                    handle, f"{k}s", v
                )  # will not work for all single => multi migrations
            if k in fallback_mv_keys:
                setattr(
                    ...
                    MV_SEPARATOR.join(v)
                    if fallback_mv
                    else MV_SEPARATOR_VISUAL.join(v),
                )
        else:
            setattr(handle, k, v)
...
def determine_image_crop(image_bytes: bytes):
    ...
    if (
        avg_dev < AVG_THRESHOLD
        and dev_red < CHANNEL_THRESHOLD
        and dev_green < CHANNEL_THRESHOLD
        and dev_blue < CHANNEL_THRESHOLD
    ):
...
def get_1x1_cover(
    ...
    if cover_crop_method == "crop":
        ...
        dominant_color = (
            get_dominant_color(pil_img) if recc_fill_color is None else recc_fill_color
        )
        pil_img = ImageOps.pad(
            pil_img, (width, width), color=dominant_color, centering=(0.5, 0.5)
        )
