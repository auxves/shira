...
def parse_datestring(datestr: str):
    ...
    if re.match(r"^\d{8}$", datestr):
        ...
        raise Exception(
            f"parse_datesting: unknown format of '{datestr}' - use YYYY-MM-DD or YYYYMMDD"
        )


def get_year(
    track: dict[str, str | int], ytmusic_album: dict[str, str | int] | None = None
):
    """:returns release_year, release_date"""
    date = {"day": "01", "month": "01", "year": ""}
    release_date = ""
    release_year = ""
    ...
    release_date = (
        datetime.datetime(
            day=int(date["day"]), month=int(date["month"]), year=int(date["year"])
        ).isoformat()
        + "Z"
    )
...
def smart_tag(list_of_keys: list[str], data_obj: dict, additional_values: list[str]):
    ...
    counts = Counter(
        cleaned_tags
    )  # count how many times a string occurs in the tags list
...
def get_youtube_maxres_thumbnail(info):
    ...
    for t in thumbs:  # try to get maxresdefault
        if t["url"] in pinged_urls:
            continue
        if t["url"].endswith("/maxresdefault.jpg") or t["url"].endswith(
            "/maxresdefault.png"
        ):
            res = ping_yt(t["url"])
    ...
    for (
        t
    ) in thumbs:  # otherwise, just take the one with the best preference but out format
        if t["url"] in pinged_urls:
            continue
...
def smart_metadata(info, cover_format="JPEG", cover_crop_method="auto"):
    ...
    md_keys = {
        ...
        "album": [],
        "year": [],
    }  # keys to check from the 'info object'. site specific.
    add_values = {
        "title": [],
        "artist": [],
        "albumartist": [],
        ...
        case _:
            if domain != "youtube.com":
                print(
                    ...
                    str(domain),
                    "using youtube extractor as fallback.",
                )
            md_keys, add_values = youtube_extractor(info)

    md["title"], others["title"] = smart_tag(
        md_keys["title"], info, add_values["title"]
    )
    md["artist"], others["artist"] = smart_tag(
        md_keys["artist"], info, add_values["artist"]
    )
    md["albumartist"], others["albumartist"] = smart_tag(
        md_keys["albumartist"], info, [md["artist"]] + add_values["albumartist"]
    )
...
def clean_title(title: str):
    ...
    title = re.sub(
        r"\*\b[A-Z ]+\b\*", "", title
    )  # remove stuff like *NOW ON ALL PLATFORMS*
    title = re.sub(
        r"(\S)\[", r"\g<1>" + " [", title, flags=re.MULTILINE
    )  # jap title whitespace fix
