...
def process_directory(
    ...
    if os.path.isfile(str(directory_or_file)):
        process_song(
            str(directory_or_file), 0, 1, fetch_complete, fetch_partial, dry_run, debug
        )
        print()
        return
...
def process_song(
    ...
    continue_partials = has_some == 0 or (
        has_some > 0 and (fetch_partial or fetch_complete)
    )
    ...
    formb_album = (
        str(handle.album) if handle.album is not None else f"{handle.title} (Single)"
    )
    ...
    if dry_run:
        ...
        if ptags is not None:
            if (
                ptags["artist"] is None
                or ptags["title"] is None
                or ptags["album"] is None
            ):
                pprint(ptags)
...
@click.command()
@click.argument(
    "input_path", type=click.Path(exists=True, file_okay=True, resolve_path=True)
)
@click.option(
    "--fetch-complete",
    "-c",
    is_flag=True,
    help=f"Fetch from MusicBrainz even if has {', '.join(MBID_TAG_KEYS)} present.",
)
@click.option(
    "--fetch-partial",
    "-p",
    is_flag=True,
    help="Fetch from MusicBrainz even if has some mb_* tags present.",
)
@click.option(
    "--dry-run",
    "-d",
    is_flag=True,
    help="Don't write to any files, just print out the mb_* tags",
)
@click.option(
    "--debug",
    "-g",
    is_flag=True,
    help="Prints out extra information for debugging. Does not imply --dry-run.",
)
def mbtag_cli(
