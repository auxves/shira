...
MBArtistCredit = TypedDict(
    "MBArtistCredit", {"name": str, "sort-name": str, "artist": MBArtist}
)
...
yeet_regexes = [leading_zero_re, title_feat_re, r","]
...
def check_bareartist_match(artist: str, a_dict: MBArtist):
    ...
    return (
        artist == a_dict["name"]
        or artist.lower() == a_dict["name"].lower()
        or artist == a_dict["sort-name"]
        or artist.lower() == a_dict["sort-name"].lower()
    )
...
def check_barealbum_match(album: str, r_dict: MBRelease):
    ...
    return (
        album == r_dict["title"]
        or album.replace("(Single)", "").strip() == r_dict["title"]
        or album.lower() == r_dict["title"].lower()
        or album.replace("(Single)", "").strip().lower() == r_dict["title"].lower()
...
def check_barealbum_match2(album: str, r_dict: MBRelease):
    ...
    return (
        album in r_dict["title"]
        or album.replace("(Single)", "").strip() in r_dict["title"]
        or album.lower() in r_dict["title"].lower()
        or album.replace("(Single)", "").strip().lower() in r_dict["title"].lower()
        or normalized_compare_regex(album, r_dict["title"], strict=False)
    )


def check_album_match(
    album: str, r_dict: MBRelease, title_match: bool, artist_match: bool
):
...
def check_title_match(title: str, r_dict: MBRecording, debug=False):
    ...
    return (
        title == r_dict["title"]
        or title.lower() == r_dict["title"].lower()
...
class MBSong:
    ...
    def __init__(
        ...
        self.req = CachedSession(
            "shira", expire_after=cache_lifetime_seconds, use_cache_dir=True
        )
        self.head = {
            "User-Agent": f"shiradl+mbtag/{shiraver} ( https://github.com/KraXen72/shira )"
        }
    ...
    def fetch_artist(self):
        ...
        params = {"query": self.artist, **self.default_params}
    ...
    def _debug_print_track(
        self, track: MBRecording, titm: bool, artm: bool, albm: bool
    ):
        if not self.debug:
            return
    ...
    def save_song_dict(self, tracks: list[MBRecording]):
        ...
        for t in tracks:
            if (
                ("artist-credit" not in t)
                or (len(t["artist-credit"]) == 0)
                or ("releases" not in t)
                or (len(t["releases"]) == 0)
            ):
                continue
    ...
    def get_date_str(self):
        ...
        if return_val is not None:
            if re.match(r"^\d{8}$", return_val) or re.match(
                r"^\d{4}-\d{2}-\d{2}$", return_val
            ):
                return return_val
    ...
    def get_mbid_tags(self):
        ...
        first_mb_artistid = (
            self.mb_artistid[0]
            if isinstance(self.mb_artistid, list)
            else self.mb_artistid
        )
    ...
    def get_mb_tags(self):
        ...
        if self.artist_dict is not None:
            artist = (
                self.artist_dict[0]["artist"]["name"]
                if isinstance(self.artist_dict, list)
                else self.artist_dict["name"]
            )
        return {
            "title": self.song_dict.get("title")
            if self.song_dict is not None
            else None,
            "artist": artist,
            "album": self.album_dict.get("title")
            if self.album_dict is not None
            else None,
        }
...
def musicbrainz_enrich_tags(
    ...
    if use_mbid_data:
        if mb.artist_dict:
            ...
            tags["albumartist"] = (
                mb.artist_dict[0]["artist"]["name"]
                if isinstance(mb.artist_dict, list)
                else mb.artist_dict["name"]
            )
        if mb.album_dict:
